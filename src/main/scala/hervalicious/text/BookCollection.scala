...
class BookCollection(availableBooks: List[BookTitle] = Books.all,
                     bookLoader: GutenbergEbookLoader = new GutenbergEbookLoader(),
                     maxSentences: Int = 3) {
  ...
  private def randomBookCombo: BookPair = {
    val books = Random.shuffle(availableBooks).take(2)
    new BookPair(books(0), books(1), bookLoader, maxSentences)
  }
